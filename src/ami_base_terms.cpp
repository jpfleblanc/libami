...
/**
 * This is a simplified AMI symbolic integration function.  It takes a starting
 * integrand defined by `g_prod_t` R0 and `ami_parms` object, and returns the
 * terms for symbolic evaluation in a simple `terms` structure.
 * @param[in] parms : `ami_parms` object, basic parameters for AMI.
 * @param[in] R0 : `g_prod_t` integrand to be processed.
 * @param[out] terms_out: Resultant `terms` containing separate AMI terms for
 * later evaluation.
 */
...
void AmiBase::terms_general_residue(term &this_term, pole_struct this_pole,
                                    terms &out_terms) {
  out_terms.clear();
...
void AmiBase::take_term_derivative(term &in_term, pole_struct &pole,
  ...
  temp; temp.g_list=in_term.g_list; temp.p_list=in_term.p_list;
...
void AmiBase::split_term(term &this_term, pole_struct this_pole,
                         term &innert_part, term &active_part) {
