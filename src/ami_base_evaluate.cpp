...
std::complex<double> AmiBase::evaluate(ami_parms &parms, R_t &R_array,
  ...
  // std::complex<double> double_check=evaluate(parms,R_array, P_array,
  // S_array, external);
  ...
  for (int i = 0; i < dim - 1; i++) {
    ...
    // need line that converts the Si_t from integers to doubles? So that the
    // dot operator has doubles both left and right entries.
...
std::complex<double> AmiBase::optimized_star(ami_parms &parms, SorF_t K,
  ...
  for (int i = 0; i < Eval_list.size(); i++) {
    ...
    for (int j = 0; j < Eval_list[i].size(); j++) {
      // std::cout<<j<<" "<<K[0][Eval_list[i][j].first]<<"
    ...
    // std::cout<<
    // std::setprecision(std::numeric_limits<boost::multiprecision::float128>::max_digits10)<<
    // i<<" "<< ksum <<" "<< std::real(this_gprod)<<"
    // "<<std::imag(this_gprod)<< " "<<std::real(term)<<" "<< std::imag(term)
    // <<std::endl;
    // }
...
std::complex<double> AmiBase::star(ami_parms &parms, SorF_t K, Ri_t R,
  ...
  for (int i = 0; i < K[0].size(); i++) {
    ...
    // if(std::fmod(std::imag(gprod),1.0)==0 && std::abs(std::imag(gprod))
    // !=0
    // ){std::cout<<"Numerical overflow has occurred in imaginary
    // part"<<std::endl; overflow_detected=true; print_output=true;
    // }
    ...
    // print_output=true;
    // file <<i<<" "<< K[0][i] <<" "<< std::real(gprod)<<"
    // "<<std::imag(gprod)<< " "<<std::real(term)<<" "<< std::imag(term)
    // <<std::endl;
  }
...
AmiBase::SorF_t AmiBase::fermi(ami_parms &parms, Pi_t Pi, ami_vars external) {
  ...
  for (int i = 0; i < Pi.size(); i++) {
    ...
    for (int j = 0; j < Pi[i].size(); j++) {
      // std::cout<<"Fermi term "<< fermi_pole(parms, Pi[i][j],
      // external)<<std::endl;
...
std::complex<double> AmiBase::fermi_pole(ami_parms &parms, pole_struct pole,
  ...
  // In order to generalize to have fermi and bose lines, here to 'sigma' needs
  // to be considered.
  ...
  // TODO: should this be improved somehow? Should frequencies know their stat
  // type? handle all but one external
  ...
  if (parms.TYPE_ == AmiBase::doubleocc) {
    if (pole.index_ == pole.alpha_.size() - 1) {
      eta++;
      // std::cout<<"Incrementing eta since this is a bosonic integral for
      // "<< pole.index_<<std::endl;
    }
  }
  ...
  if (E == zero && sigma == -1) { // && pole.der_==0    Not sure if pole
    ...
    // std::cout<<"Bosonic function at zero energy - must vanish, setting to
    // zero"<<std::endl;
    if (drop_bosonic_diverge) {
      // std::cout<<"Bosonic function at zero energy - must vanish, setting
      // to zero"<<std::endl;
  ...
  // std::cout<<"Evaluated Fermi derivative function and got "<<output<< " at
  // energy "<< E <<" with der="<<pole.der_<<" and sigma="<<sigma<<std::endl;

  // std::cout<<"Fermi Pole Term evaluated to "<< output << " at energy "<<
  // E<<" with der="<<pole.der_<<" with sigma "<<sigma<< " betaE is "<<
  // beta*E<<" in exponent "<< std::exp(beta*(E))<< std::endl;
...
std::complex<double> AmiBase::fermi_bose(int m, double sigma, double beta,
  ...
  // std::cout<<"Fermi Pole output evaluated to "<< output << " at energy "<<
  // E<<" with sigma "<<sigma<< " betaE is "<< beta*E<<" in exponent "<<
  // std::exp(beta*(E))<< std::endl;
...
std::complex<double> AmiBase::eval_gprod(ami_parms &parms, g_prod_t g_prod,
  ...
  for (int i = 0; i < g_prod.size(); i++) {
    ...
    // TODO: Right here, if the denom==0 still, then the R entry was empty,
    // so regulate the next section, eps -> eps+i0+

    std::complex<double> zero(0, 0);
    std::complex<double> im(0, 1);

    // std::cout<<"Energies"<<std::endl;
    // Unsure. should this be -=? given that my epsilon is the positive
    // quantity?
    for (int a = 0; a < g_prod[i].eps_.size(); a++) {
      ...
      // if(verbose){
      // std::cout<<a<<" "<<g_prod[i].eps_[a]<<" ext "<<
      // external.energy_[a]<<"
      // "<<std::endl;
      // }
    }
    ...
    // std::cout<<"alphadenom:  "<<alphadenom<<"  epsdenom:  "<<epsdenom<<"
...
double AmiBase::frk(int r, int k) {
  ...
  // std::cout<<"Evaluating Frk function "<<r<<" "<<k<<" =
  // "<<output<<std::endl;
